...
# ----------------------------------------------------------------------
# 진짜 학생 글 무작위 불러오기 API 엔드포인트 및 환경변수 정의
# ----------------------------------------------------------------------
DATA_API_URL = os.getenv("DATA_API_URL", "https://gist.githubusercontent.com/user/placeholder/raw/student_essays.json")

def get_mock_student_essays() -> List[Dict[str, Any]]:
    return [
        {
            "id": "GWRW2400000001",
            "text": "대학 서열화 문제는 오늘날 고등교육의 질적 발전을 가로막는 심각한 요인 중 하나이다. 우리는 반드시 대학 서열 구조를 타파하고 지방 대학에 대한 적극적인 투자를 가짐에 틀림없다. 하지만 지방 대학을 살리는 구체적 정책이나 재정 확대 예산이 턱없이 부족하여 현실은 절대 녹록지 않다. 왜냐하면 수도권 중심의 자원 집중 현상이 오랜 기간 누적되었기 때문이다. 따라서 교육부는 단순한 평가 지표 개선에 그치지 말고 지역 균형 발전을 위한 전폭적 재정 지원을 전개해야 할 필요가 있다."
        },
        {
            "id": "GWRW2400000002",
            "text": "생성형 AI를 교육 현장에 전면 도입하는 문제는 기술 혁신과 학습권 보장이라는 측면에서 긍정적으로 보인다. 그러나 학생들이 스스로 생각하는 사유의 힘을 거세당할 수 있다는 점에서 위험성이 대단히 높다. AI가 제공하는 완벽하게 다듬어진 정리 자료는 단기 성적 향상에는 유용하게 보이지만, 결국 주체적인 비판적 성찰을 저해할 가능성이 높기 때문이다. 따라서 우리는 교육용 AI 활용 가이드를 시급히 구축하고 비판적 리터러시를 동시에 교육해야만 한다."
        }
    ]

@app.get("/api/fetch-student-essay")
def fetch_student_essay(essay_id: str = None):
    try:
        ...
        with urllib.request.urlopen(req, timeout=5) as response:
            ...
            if isinstance(data, list) and len(data) > 0:
                if essay_id:
                    essay = next((x for x in data if str(x.get("id")) == essay_id), None)
                    if essay:
                        return essay
                return random.choice(data)
    except Exception as e:
        print(f"[SYSTEM] 원격 API 연동 실패 ({str(e)}). Fallback Mock 데이터를 반환합니다.")
    
    essays = get_mock_student_essays()
    if essay_id:
        essay = next((x for x in essays if x["id"] == essay_id), None)
        if essay:
            return essay
        raise HTTPException(status_code=404, detail="해당 ID의 에세이를 찾을 수 없습니다.")
...
def analyze_essay(request: AnalyzeRequest):
    ...
    # 종합 평균이 B- 이하(3.5 미만)로 낮은 대역일 때 칭찬 미사여구를 자제하고 객관적 보완 가이드로 필터링
    ...
    if total_average >= 4.5:
        grade_commentary = "독자적인 학술적 문제의식과 역동적인 삼단 논증 호흡이 깊이 있게 조화된 훌륭한 논증문"
    elif total_average >= 4.0:
        grade_commentary = "논리가 정밀하며 학술문으로서의 기본 장르 관습을 안정적으로 준수한 논증문"
    elif total_average >= 3.5:
        grade_commentary = "기본적인 논리 구조를 양호하게 갖추었으며 학술적 표현을 자연스럽게 활용한 논증문"
    elif total_average >= 3.0:
        grade_commentary = "전반적인 흐름은 형성되었으나 문장 구조의 다양성과 단어 배치의 활력을 보완하여 완성도를 더할 수 있는 논증문"
    elif total_average >= 2.5:
        grade_commentary = "기본 어법과 문맥 연결은 성립하나 구어체나 번역투를 정돈하여 학술적 장르 격식을 보강하기를 권장하는 논증문"
    else:
        grade_commentary = "글의 유기적 구성과 문장의 다양성을 재검토하여 주체적인 흐름으로 고쳐 쓰기를 추천하는 논증문"
